...
export async function loader() {
  return await customFetch
    .get("/section/reading")
    .then((data) => data.data.readingQuestions[0].sectionD);
}
...
const Reading3 = () => {
  ...
  const formatArticleOptions = (array, index) => {
    const doc = `<select
      name="article_answer[${index}]"
  ...
  async function submit(formData) {
    ...
    let score = 0;
    for (let index = 1; index <= questions.article_questions.length; index++) {
      correct_options.push(questions.article_questions[index - 1].answerText);
      answer_array.push(formData.get(`article_answer[${index}]`));
    }
  ...
  function submitSession() {
    ...
    // window.location.href = "/reading-4";
  }
  function prev() {
          ...
          <div className="h-[600px] w-full  bg-[#EDEDED] overflow-scroll">
            <div className="max-h-[600px] grid grid-cols-2 gap-6">
              <div className="h-[500px] overflow-auto my-10 py-4 mx-4 xl:mx-8">
                ...
                {questions.article_paragraphs.map((item) => (
                  <p className="indent-4 text-[13px] text-[#525866]">{item}</p>
                ))}
              </div>
              <div className="h-[500px] overflow-auto my-10 py-4 mx-4 xl:mx-8">
                ...
                <form action={submit}>
                  ...
                  <div className="border  ml-10 rounded-lg p-4 max-w-[400px] mb-4">
                    <ol class="list-decimal px-4 text-[13px] text-[#525866]">
                      {questions.article_questions.map((item) => (
                        <li
                          key={item.id}
                          className=" mb-2 mt-1 leading-9"
                          dangerouslySetInnerHTML={{
                            __html: item.question.replace(
                              "______",
                              formatArticleOptions(item.options, item.index)
                            ),
                          }}
                        ></li>
                      ))}
