...
export async function loader() {
  return await customFetch
    .get("/section/reading")
    .then((data) => data.data.readingQuestions[0].sectionB);
}
...
const Reading3 = () => {
  ...
  async function submit(formData) {
    ...
    let score = 0;
    for (let index = 0; index < questions.email_questions.length; index++) {
      correct_options.push(questions.email_questions[index].answerText);
      answer_array.push(formData.get(`email_answer[${index}]`));
    }
    ...
    for (let index = 0; index < questions.other_questions.length; index++) {
      correct_options_b.push(questions.other_questions[index].answerText);
      answer_array_b.push(formData.get(`other_answer[${index}]`));
    }
  ...
  function submitSession() {
    ...
    window.location.href = "/reading-3b";
  }
  function prev() {
        ...
        <div className=" max-w-[1086px] w-full z-10 m-auto mt-25">
          <div className="h-[67px] bg-[#200943] shadow-md flex items-center justify-between text-lg shadow-black mt-10 xl:mt-0">
            <p className="text-white text-lg font-bold leading-17 ml-8">
              Practice Test A - Reading Part 2: Reading to Apply a Diagram
            </p>
          ...
          <div className="h-[600px] w-full  bg-[#EDEDED] overflow-scroll">
            <div className="max-h-[600px] grid grid-cols-2 gap-6">
              <div className="h-[500px] overflow-auto my-10 py-4 mx-4 xl:mx-8">
                ...
                <p className="indent-4 text-[13px] text-[#525866]"></p>
              </div>
              <div className="h-[500px] overflow-auto my-10 py-4 mx-4 xl:mx-8">
                ...
                <form action={submit}>
                  <div class="inline-flex items-center mt-4 mb-4">
                    ...
                    <p className="text-sm text-gray-900 max-w-[390px]">
                      Read the following email message about the diagram on the
                      left. Complete the email by filling in the blanks. Select
                  ...
                  <div className="border  ml-10 rounded-lg p-4 max-w-[400px] mb-4">
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: questions.email
                          .replace(
                            "[1] ______",
                            formatLetterOptions(questions.email_questions[0], 0)
                          )
                          .replace(
                            "[2] ______",
                            formatLetterOptions(questions.email_questions[1], 1)
                          )
                          .replace(
                            "[3] ______",
                            formatLetterOptions(questions.email_questions[2], 2)
                          )
                          .replace(
                            "[4] ______",
                            formatLetterOptions(questions.email_questions[3], 3)
                          )
                          .replace(
                            "[5] ______",
                            formatLetterOptions(questions.email_questions[4], 4)
                          ),
                      }}
                    ></div>
                  </div>
                  ...
                  <div className="border  ml-10 rounded-lg p-4 max-w-[400px] mb-4">
                    <ol class="list-decimal px-4 text-[13px] text-[#525866]">
                      {questions.other_questions.map((item) => (
                        <li key={item.id} className=" mb-2 mt-1">
                          ...
                          <select
                            ...
                            name={`other_answer[${questions.other_questions.indexOf(
                              item
                            )}]`}
